...
export class LocaleUtil {
    ...
    },
    {
        "LocaleString": "es-DO",
        "Country": "DO",
        "DisplayCountry": "Dominican Republic",
        "DisplayLanguage": "Spanish",
    ...
    {
        "LocaleString": "no-NO-NY",
        "Country": "NO",
        "DisplayCountry": "Norway",
        "DisplayLanguage": "Norwegian",
        "DisplayName": "Norwegian (Norway,Nynorsk)",
        "DisplayVariant": "Nynorsk",
        "ISO3Country": "NOR",
        "ISO3Language": "nor",
        "Language": "no",
        "Variant": "NY"
    ...
    },
    {
        "LocaleString": "sr-BA",
        "Country": "BA",
        "DisplayCountry": "Bosnia and Herzegovina",
        "DisplayLanguage": "Serbian",
    ...
    },
    {
        "LocaleString": "sr-CS",
        "Country": "CS",
        "DisplayCountry": "Serbia and Montenegro",
        "DisplayLanguage": "Serbian",
    ...
    {
        "LocaleString": "th-TH-TH",
        "Country": "TH",
        "DisplayCountry": "Thailand",
        "DisplayLanguage": "Thai",
        "DisplayName": "Thai (Thailand,TH)",
        "DisplayVariant": "TH",
        "ISO3Country": "THA",
        "ISO3Language": "tha",
        "Language": "th",
    ...
    private static _default_locale = 'en-us';
    static default() {
        return this._default_locale;
    }

    private static filterLocale(accept_language: string): Locale | undefined {
        // en-US,en;q=0.9,de;q=0.8
        if (!accept_language) {
            accept_language = this._default_locale;
        }
        const possible_locales = accept_language.split(",").map(c => {
            if (c.includes(';')) {
                return c.split(";")[0];
            } else {
                return c;
        ...
        let matchedLocale: Locale | undefined = undefined;
        for (const possible_locale of possible_locales) {
            let _matchedLocale = this._locales.find(c => c.LocaleString.toLowerCase() == possible_locale.toLowerCase());
            if (_matchedLocale) {
                matchedLocale = _matchedLocale;
                break;
            } else {
                _matchedLocale = this._locales.find(c => c.Language.toLowerCase() == possible_locale.split("-")[0].split("_")[0].toLowerCase());
                if (_matchedLocale) {
                    matchedLocale = _matchedLocale;
                    break;
                }
    ...
    static fromAcceptLanguage(accept_language: string): Locale | undefined {

        let matchedLocale = this.filterLocale(accept_language);
        if (!matchedLocale) {
            matchedLocale = this.filterLocale(this._default_locale);
        }
        return matchedLocale;
    ...
    static validate(locale: string) {
        let matchedLocale = this._locales.find(c => c.LocaleString.toLowerCase() == locale.toLowerCase());
        if (!matchedLocale) {
            throw new ErrorEntity({ http_code: HttpStatus.EXPECTATION_FAILED, error: "invalid_request", error_description: "given locale is not supported, supported values : " + this._locales.map(c => c.LocaleString.toLowerCase()).join(",") });
        }
